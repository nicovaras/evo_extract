...
const HTTP_URL = (import.meta as { env?: { VITE_HTTP_URL?: string } }).env?.VITE_HTTP_URL ?? 'http://localhost:2567';
...
export class LobbyScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.add.text(cx, 135, 'Lobby', { ...STYLE_LABEL, fontSize: '22px', color: '#ffffff' }).setOrigin(0.5);

    // ── Code input (HTML overlay) ─────────────────────────────────────────────
    this.add.text(cx - 130, 220, 'Código de sala:', STYLE_LABEL).setOrigin(0, 0.5);
    this.codeInput = this._createInput(cx, 250, 'Ej: ABC123');

    // ── Buttons ───────────────────────────────────────────────────────────────
    this._createButton(cx - 95, 330, 'Crear sala', '#1a6b3a', () => this._handleCreate());
    this._createButton(cx + 95, 330, 'Unirse', '#1a3a6b', () => this._handleJoin());
  ...
  private async _handleCreate(): Promise<void> {
    try {
      ...
      await this.network.connect(roomId);
    } catch {
  ...
  private async _handleJoin(): Promise<void> {
    ...
    try {
      ...
      await this.network.connect(data.roomId);
    } catch {
  ...
  private _setStatus(message: string, color: string): void {
    ...
    this.statusText.setText(message).setStyle({ ...STYLE_STATUS, color }).setVisible(true);
  }

  // ─── DOM Helpers ─────────────────────────────────────────────────────────────

  private _createInput(x: number, y: number, placeholder: string): HTMLInputElement {
  ...
  private _createButton(x: number, y: number, label: string, bg: string, onClick: () => void): void {
    const btn = this.add
      .text(x, y, label, {
        fontSize: '20px',
        color: '#ffffff',
